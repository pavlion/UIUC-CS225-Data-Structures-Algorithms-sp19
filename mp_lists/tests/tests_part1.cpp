...
TEST_CASE("List::insertFront size", "[weight=2][part=1][valgrind]") {
    ...
    for (unsigned i = 0; i < 10; i++) { list.insertFront(i); }
	cout<<"List::insertFront size"<<endl;
...
TEST_CASE("List::waterfall with int", "[weight=20][part=1][valgrind]") {

    List<int> list;
	int arr[8]={1,3,5,7,2,6,4,8};
    for (int i = 1; i < 9; i++) { list.insertBack(i); }
	...
	int i;
	for (iter = list.begin(), i = 0 ; i < 9 && iter!=list.end(); i++,++iter)
		if(arr[i]!=*iter) val = false;
...
TEST_CASE("List::split harder", "[weight=5][part=1][valgrind]") {
    List<int> list;
