 ...
 *
 * The SquareMaze class should be declared and defined in maze.h and maze.cpp, respectively.
 * Each SquareMaze object will represent a randomly-generated square maze and its solution.
 * Note that by “square maze” we mean a maze in which each cell is a square;
 * the maze itself need not be a square.
 * As always, we recommend reading the whole specification before starting.
 *
 * setWall and canTravel You should triple check that setWall and canTravel function exactly according to spec,
 * as an error in these functions will not be caught by making your own mazes
 * but can cost you a majority of the points during grading.
 *
 * @author Austin Tsai
 */
...
class SquareMaze
{
  public:
	 ...
	 *
	 * Hints: You only need to store 2 bits per square:
	 * the "down" and "right" walls.
	 ...
	 * It will also be used by the grading program to verify that
	 * your maze is a tree that occupies the whole grid,
	 ...
	 * @param x	 :	   The x coordinate of the current cell
	 * @param y	 :	   The y coordinate of the current cell
	 * @param dir:	   Either 0 (right) or 1 (down), which specifies which wall to set (same as the encoding explained in canTravel). You only need to support setting the bottom and right walls of every square in the grid.
	 ...
	 *
	 * Select the square in the bottom row with the largest distance
	 * from the origin as the destination of the maze.
	 * solveMaze() returns the winning path from the origin to
	 * the destination as a vector of integers,
