...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
	...
	if (!subroot->elements.empty() &&(subroot->elements[first_larger_idx]).key == key) 
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
	...
	cout<<"*mid_elem_itr:"<<((*mid_elem_itr)==child->elements[mid_elem_idx])<<endl;
	...
	(new_left->elements).erase(mid_elem_itr+1, (new_left->elements).end());
	(new_left->children).erase(mid_child_itr+1,(new_left->children).end());
	*/
}
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
	...
	//	return (subroot->elements[first_larger_idx]).value;
	/*
	if ( !subroot->elements.empty() && subroot->elements[first_larger_idx] == pair){
		return;
	}
	*/
	...
	if (subroot->is_leaf == true){
		(subroot->elements).insert(
								  (subroot->elements).begin()+first_larger_idx
								  ,pair);
		return;
	}
