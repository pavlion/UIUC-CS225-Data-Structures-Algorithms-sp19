...
class BinaryTree
{
    ...
    private:
         ...
         * @param subRoot The current node in the recursion
         */
        void _mirror_helper(Node* subRoot);
		
		/** ADDED HELPER FUNCTION!
         * Private helper function for the public isOrderedRecursive function.
         * @param subRoot The current node in the recursion
		 * @param min: store the current min value
		 * @param max: store the current max value
         */
		bool _isOrderedRecursive_helper(Node* subRoot, T min, T max) const;
		T maxOfLeft(Node* subRoot) const;
		T minOfLeft(Node* subRoot) const;
		/** ADDED HELPER FUNCTION!
         * Private helper function for the public getPaths(std::vector<std::vector<T>>& paths)  function.
         * @param subRoot		The current node in the recursion
		 * @param totalPaths 	The total paths
		 * @param totalPaths 	The current trailing path
         */		
		void _getPaths_helper(Node* subRoot, 
         ...
         * @param currDistance	The distance from root that we're currently in
		 * @param subRoot 		The current node we're in
		 * @param sum 			The sum of all distances
         */			
		void _sumDistances_helper(int currDistance, Node* subRoot, int &sum) const;
