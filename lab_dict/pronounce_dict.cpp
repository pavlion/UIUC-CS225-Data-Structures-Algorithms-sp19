...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
	...
	if (dict.count(s1) == 0 || dict.count(s2) == 0) return false;

    const vector<string>& v1 = dict.at(s1);
    const vector<string>& v2 = dict.at(s2);
    //if ( *(dict.find(s1)) == *(dict.find(s2))) return true;
