                            ...
                            queue<TreeNode*>& mergeQueue)
{
	...
	if ( singleQueue.empty() && mergeQueue.empty() )
		...
		TreeNode* t = mergeQueue.front();
		mergeQueue.pop();
		return t;
	}
	else if ( mergeQueue.empty() ){
		TreeNode* t = singleQueue.front();
		singleQueue.pop();		
		return t;
	}
	
	TreeNode* t1 = singleQueue.front();
	TreeNode* t2 = mergeQueue.front();
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
	...
	while(  !(singleQueue.size() == 0 && mergeQueue.size() == 1) &&
			!(singleQueue.size() == 1 && mergeQueue.size() == 0) ){
			
		TreeNode* t1 = removeSmallest(singleQueue, mergeQueue);
		TreeNode* t2 = removeSmallest(singleQueue, mergeQueue);
		TreeNode* t = new TreeNode(t1->freq.getFrequency() + t2->freq.getFrequency());
		t->left = t1;
		t->right = t2;
		mergeQueue.push(t);
	}
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if ( bfile.hasBits() ) {
		...
		if ( b == 1 ){
			...
			Frequency freq(c, 0);
			TreeNode* t = new TreeNode(freq);// it's a leaf
			return t;
		}
